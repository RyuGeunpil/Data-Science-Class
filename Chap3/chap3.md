...
age=c(32,25,18,39),
height=c(170, 175, 168, 180),
weight=c(63, 66, 59, 70)) #데이터프레임 생성

class(df_1) #데이터 형식 파악
str(df_1) #데이터 구조 파악
...
df_1[c(1,3),] #1과 3행의 값과, 전체 열의 값
```

**데이터프레임 구조 달러($) 인덱싱** 
```
df_1$height #height 변수 지정
df_1$height >= 175 #조건에 맞는 값을 보여줌

df_1$name[df_1$height >= 175]  #조건에 맞는 값의 name을 보여줌

df_1$height[df_1$height < 175] <- 160 #조건에 해당하는 값을 175로 변경
df_1
...
my_list[[2]] #2번재 주머니 인덱싱

class(my_list[[3]]) #3번째 주머니 인덱싱
```

**리스트 구조 달러($) 인덱싱** 
```
str(my_list) #리스토 구조 파악
my_list$num #num 변수 인덱싱
my_list$num[2] #num 변수의 2번째 값
my_list$df$x  #df 데이터프레임에서 x 변수값 인덱싱
my_list$new_var <- c(10, 20, 30) #새로운 변수 생성
```
